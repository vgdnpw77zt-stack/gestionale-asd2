...
if not MARKER.exists():
    def patch_inbound(s):
        ...
        if old_cell not in s:
            raise RuntimeError('R9 document type cell anchor missing')
    ...
    def patch_tesserati(s):
        ...
        if css_anchor not in s:
            raise RuntimeError('R9 tesserato css anchor missing')
        ...
        if card_anchor not in s:
            raise RuntimeError('R9 tesserato card anchor missing')
    ...
    # Backfill non-destructive metadata links for existing Autopilot documents.
    if DB.exists():
